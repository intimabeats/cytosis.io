...
export class Renderer {
  ...
  drawOffscreenIndicator(worldPos: {x: number, y: number}, color: string): void {
    ...
    if (!indicatorX || indicatorX < borderPadding || indicatorX > this.canvas.width - borderPadding) {
      if (screenPos.x < 0) {
        // Borda esquerda
    ...
    ctx.beginPath();
    ctx.moveTo(
      indicatorX + Math.cos(angle) * indicatorRadius * 1.5,
      indicatorY + Math.sin(angle) * indicatorRadius * 1.5
    );
    ctx.lineTo(
      indicatorX + Math.cos(angle + 2.5) * indicatorRadius,
      indicatorY + Math.sin(angle + 2.5) * indicatorRadius
    );
    ctx.lineTo(
      indicatorX + Math.cos(angle - 2.5) * indicatorRadius,
      indicatorY + Math.sin(angle - 2.5) * indicatorRadius
