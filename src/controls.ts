// src/controls.ts - Updated for better mouse tracking
...
export class Controls {
  ...
  private setupEventListeners(): void {
    // Mouse move event - IMPROVED for more responsive tracking
    this.canvas.addEventListener('mousemove', (e) => {
      ...
      // Dispatch a custom event for immediate mouse position updates
    ...
    // Touch events for mobile - IMPROVED for better mobile experience
    this.canvas.addEventListener('touchstart', (e) => {
      ...
      // Also update mouse position on touch start
      const screenPos: Vector2D = {
        x: touch.clientX - rect.left,
        y: touch.clientY - rect.top
      };
    ...
    this.canvas.addEventListener('touchmove', (e) => {
      ...
      // Dispatch mouse move event for immediate updates
      const mouseMoveEvent = new CustomEvent('game-mouse-move', {
        detail: {
          position: this.inputState.mousePosition
        }
    ...
    // Add listener for window resize to update mouse position
    window.addEventListener('resize', () => {
      // Reset mouse position on resize to prevent issues
